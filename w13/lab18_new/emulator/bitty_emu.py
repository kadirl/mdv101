    ...
    def load_from_file(self, path):
        with open(path) as f:
            for i, line in enumerate(f):
                ...
                line = line.strip()
                if line: self.data[i] = int(line, 16)
...
class LDSTInstruction(Instruction):
    def execute(self, emulator):
        f = self.decode_fields()
        if f["ls"] == 0:
            emulator.set_reg(f["rx"], emulator.memory[emulator.get_reg(f["ry"])])
        else:
            emulator.memory[emulator.get_reg(f["ry"])] = emulator.get_reg(f["rx"])
        emulator.pc += 1
...
class BittyEmulator:
    ...
    def _load_instructions(self, path):
        with open(path) as f:
            for line in f:
                line = line.strip()
                if line: self.instructions.append(int(line, 16))
